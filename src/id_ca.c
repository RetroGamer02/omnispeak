...
void CAL_CacheSprite(int chunkNumber, uint8_t *compressed)
{
	...
	case 4:
		...
		break;
	default:
		printf("CAL_CacheSprite: Bad shifts number!");
	}
}
...
mm_ptr_t CA_GetGrChunk(int base, int index, const char *chunkType, bool required)
{
	...
	if ((chunk < 0) || (chunk >= CA_MAX_GRAPH_CHUNKS))
		printf("tried to render a graphics chunk with invalid ID");
	...
	if (!result)
	{
		...
		result = ca_graphChunks[chunk];
		if (!result && required)
			printf("tried to render an uncached graphics chunk");
	}
	return result;
}
...
void CA_CacheGrChunk(int chunk)
{
	...
	do
	{
		...
		if (curRead < 0)
		{
			printf("Error reading compressed graphics chunk.");
		}
		read += curRead;
...
void CAL_SetupMapFile(void)
{
	...
	if (!CA_LoadFile("TILEINFO.EXT", (void **)(&ti_tileInfo), 0))
	{
		...
		if (mapHeadFileSize <= sizeof(*ca_MapHead))
			printf("Can't open TILEINFO file, and MAPHEAD file lacks tileinfo data!");
...
void CA_CacheMap(int mapIndex)
{
	...
	if (!CA_MapHeaders[mapIndex])
	{
		...
		if (headerOffset <= 0)
		{
			printf("CA_CacheMap: Tried to load a non-existant map!");
		}
		...
		if (mapHeaderSize != 1)
			printf("Couldn't read map header from GAMEMAPS!");
#ifdef CK_CROSS_IS_BIGENDIAN
		for (int plane = 0; plane < CA_NUMMAPPLANES; ++plane)
		{
	...
	for (int plane = 0; plane < CA_NUMMAPPLANES; ++plane)
	{
		...
		do
		{
			...
			if (curRead < 0)
			{
				printf("Error reading compressed map plane.");
			}
			read += curRead;
...
void CA_CacheAudioChunk(int16_t chunk)
{
	...
	if (compressed <= BUFFERSIZE)
	{
		...
		if (readSize != compressed)
			printf("Couldn't read compressed audio chunk!");
		source = buffer;
	}
	else
	{
