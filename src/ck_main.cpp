...
#ifdef WITH_SDL
#include <SDL/SDL.h> // For main (SDL_main) function prototype
#endif
...
int main(int argc, char *argv[])
{
	...
	DIR* dir;
	dir = opendir("3ds/OmniSpeak");
	if (dir) {
		closedir(dir);
	} else if (ENOENT == errno) {
		//printf("Raptor directory error: %d\n" ,errno);
		mkdir("3ds/OmniSpeak", 0700);
		cp("3ds/OmniSpeak/OMNISPK.CFG","romfs:/OMNISPK.CFG");
	} else {
		printf("OmniSpeak directory unknown error.\n");
	}

	dir = opendir("3ds/OmniSpeak/KEEN");
	if (dir) {
		closedir(dir);
	} else if (ENOENT == errno) {
		//printf("Raptor directory error: %d\n" ,errno);
		mkdir("3ds/OmniSpeak/KEEN", 0700);
	} else {
		printf("OmniSpeak Keen directory unknown error.\n");
	}
	#endif
