...
SDL_AudioDeviceID sd_sdl3_audioDevice;
...
int SD_SDL_t0InterruptThread(void *param)
{
	while (SD_SDL_useTimerFallback)
	{
		...
		{
			SDL_LockAudioDevice(sd_sdl3_audioDevice);
			SDL_t0Service();
			SDL_UnlockAudioDevice(sd_sdl3_audioDevice);
			if (!SD_SDL_WaitTicksSpin)
				SDL_CondBroadcast(SD_SDL_TimerConditionVar);
...
void SD_SDL_Startup(void)
{
	...
	sd_oplDelaySamples = CFG_GetConfigInt("sd_oplDelaySamples", desiredSpec.freq / 10000);
	...
	{
		desiredSpec.freq = CFG_GetConfigInt("sampleRate", 49716); // OPL rate
		desiredSpec.format = AUDIO_S16;
		desiredSpec.channels = CFG_GetConfigInt("audioChannels", 2);
		...
		desiredSpec.samples = CFG_GetConfigInt("audioBufferSamples", 512);
		if (!sd_sdl_queueAudio)
			desiredSpec.callback = SD_SDL_CallBack;
		desiredSpec.userdata = NULL;
		sd_sdl3_audioDevice = SDL_OpenAudioDevice(NULL, 0, &desiredSpec, &SD_SDL_AudioSpec, SDL_AUDIO_ALLOW_FREQUENCY_CHANGE | SDL_AUDIO_ALLOW_SAMPLES_CHANGE);
		...
		SDL_PlayAudioDevice(sd_sdl3_audioDevice);
	}
...
void SD_SDL_Shutdown(void)
{
	...
	{
		SDL_CloseAudioDevice(sd_sdl3_audioDevice);
		SDL_QuitSubSystem(SDL_INIT_AUDIO);
		SD_SDL_AudioSubsystem_Up = false;
	}
	if (SD_SDL_useTimerFallback)
