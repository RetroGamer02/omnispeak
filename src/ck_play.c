...
CK_object *CK_GetNewObj(bool nonCritical)
{
	if (!ck_freeObject)
	{
		if (nonCritical)
		{
			...
			printf("GetNewObj: No free spots in objarray!");
	}
...
void CK_RemoveObj(CK_object *obj)
{
	if (obj == ck_keenObj)
	{
		printf("RemoveObj: Tried to remove the player!");
	}
...
void StartMusic(int16_t level)
{
	...
	if ((level >= 20) && (level != -1))
	{
		printf("StartMusic() - bad level number");
	}
	SD_MusicOff();
